...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Set the width to the window width minus padding (e.g., 30)
    const float w = window->Size.x - 30;
    ...
    ImVec2 size = CalcItemSize(ImVec2(w, h), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    // Track the alpha value and state for fade-in and fade-out
    static std::map<ImGuiID, float> alphaMap;
    static std::map<ImGuiID, bool> fadeInMap;

    auto it_alpha = alphaMap.find(id);
    if (it_alpha == alphaMap.end())
    {
        alphaMap[id] = 0.0f;
        fadeInMap[id] = false;
        it_alpha = alphaMap.find(id);
    }

    // Update alpha value
    const float fadeSpeed = 0.005f; // Adjust the speed of the fade effect
    if (hovered || held)
    {
        fadeInMap[id] = true;
    }
    ...
    if (fadeInMap[id])
    {
        it_alpha->second += fadeSpeed;
        if (it_alpha->second > 0.1f)
            it_alpha->second = 0.1f;
    }
    else
    {
        it_alpha->second -= fadeSpeed;
        if (it_alpha->second < 0.05f)
            it_alpha->second = 0.05f;
    }

    // Render
    ImVec4 button_color = ImVec4(1.0f, 1.0f, 1.0f, it_alpha->second); // 255, 255, 255, alpha
    const ImU32 col = GetColorU32(button_color);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, false, 5.0f);
...
bool ImGui::Tab(const char* label, ImTextureID icon, bool active)
{
    ...
    window->DrawList->AddRectFilled(rect.Min + ImVec2(10,5), rect.Min +  ImVec2(150,35), ImColor(it_anim->second.bg), 3);
    ...
    window->DrawList->AddText(var->assets.font[1], 18, rect.Min + ImVec2(40, 10), ImColor(it_anim->second.text), label);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}
bool ImGui::TextLine(const char* label)
{
    ...
    window->DrawList->AddText(var->assets.font[1], 18, rect.Min + ImVec2(10, 5), ImColor(140, 140, 140), label);
    window->DrawList->AddRectFilled(rect.Min + ImVec2(CalcTextSize(label).x,13), rect.Min + ImVec2(150,15), ImColor(15, 15, 15), 5);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.framecol = ImLerp(it_anim->second.framecol, *v ? ImColor(250, 121, 58) : hovered ? ImColor(30,30,30) : ImColor(25,25,25), g.IO.DeltaTime * 4.f);
    ...
    if (label_size.x > 0.f)
        ImGui::PushFont(var->assets.font[2]);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 8), frame_bb.Min + ImVec2(it_anim->second.slow - 7, 16.5f), ImColor(250, 121, 58), 100, ImDrawFlags_RoundCornersLeft);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    else
    {
        ...
        ImGui::PushFont(var->assets.font[5]);
